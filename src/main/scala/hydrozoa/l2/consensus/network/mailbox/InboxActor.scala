 ...
 *
 * NOTE: You should NOT call Actor.create on this directly! Use the [[create]] method from the companion object
...
object InboxActor:
     ...
     * @return
     */
    def create(peerId: PeerId, dbWriter: ActorRef[DBWriterActor], transmitter: ActorRef[TransmitterActor]): ActorRef[InboxActor] =
        ActorWithTimeout[InboxActor, Unit](timeout = 5.second,
            action = (inbox => inbox.transmitter.tell(_.confirmMatchIndex(peerId, inbox.matchIndex))))
            .create(InboxActor(peerId, dbWriter, transmitter))