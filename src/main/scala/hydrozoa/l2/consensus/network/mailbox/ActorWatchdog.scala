...
/** An interface that actors with a watchdog timer should provide.
  */
trait Watchdog:
    /** Called with `tell` every [[WatchdogTimeout]] in scope, exceptions must be handled by
      * the wrapped actor
      */
    def wakeUp(): Unit

object ActorWatchdog:
    /** Creates a new actor and watchdog thread that wakes it up every [[WatchdogTimeout]] in
      * scope.
      */
