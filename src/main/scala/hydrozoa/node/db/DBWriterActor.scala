...
object DbWriter:
    ...
    case class Config(
        outgoingMessagePersistedCallbacks: List[OutgoingMessagePersisted => Unit] = List.empty,
...
trait DBWriterActor(config: DbWriter.Config):
    ...
    /** Save an outgoing message and return the message ID assigned to it.
      ...
      */
    def persistOutgoingMessage(
        msg: ProtocolMsg
    ): MsgId[Outbox] // TODO: change this to OutgoingMessagePersisted
    ...
    /** Save an incoming message. Since we have a natural primary key [[InMsgId]], it doesn't return
      ...
      * @return
      */
    def persistIncomingMessage(
        peer: PeerId,
        msg: MailboxMsg[Inbox]
    ): Unit // TODO: change this to IncomingMessagePersisted
