...
/** Requests that comm actors should collectively rebroadcast to the other head peers via
  * [[NewMsgBatch]].
  */
...
/** A ledger event submission is constructed by taking a ledger event, timestamping it, and creating
  * a deferred cell where the outcome of the event can later be placed. The intention is for the
  * fiber handling the user's event submission to construct this [[SubmitLedgerEvent]] and send it
  * to the ledger event actor.
  */
...
/** The block actor announces a new block.
  ...
  *   The event number for each peer that the block creator had processed at the moment the block
  ...
  *   The set of deposits that will never be absorbed into the head's treasury because they do not
  *   fulfill the absorption criteria.
  */
...
/** A peer's block actor announces its acknowledgement of an L2 block. When a peer's block actor
  * acknowledges a block and receives all other peers' acknowledgement of the block, then the peer
  * can consider the block to be confirmed by multisig consensus. Note that for major and final
  * blocks, two rounds of acknowledgements are needed to confirm.
  */
...
final case class ConfirmBlock(
    id: BlockId
) extends CardanoLiaisonReq,
      TransactionSequencerReq,
      PersistedReq

/** Request by a comm actor to its remote comm-actor counterpart for a batch of events, blocks, or
  * block acknowledgements originating from the remote peer.
  * @param id
...
final case class TerminatedTransactionSequencer(ref: NoSendActorRef[IO])
    extends MultisigRegimeManagerReq

final case class TerminatedCardanoBackend(ref: NoSendActorRef[IO]) extends MultisigRegimeManagerReq

final case class TerminatedPersistenceActor(ref: NoSendActorRef[IO])
    extends MultisigRegimeManagerReq
