...
/** Placeholder for the head-side block-stack actor.
  ...
  * (evacuation-map diffs, payout obligations) supplied by [[hydrozoa.multisig.ledger.joint.JointLedger]],
  * decides block-stack boundaries via the slow-consensus leadership schedule, and drives
  * [[SlowConsensusActor]] to multisign each stack's necessary effects.
  *
  * This file is the parking lot for the effect-construction code (`mkBlockEffectsIntermediate`,
  * settlement / fallback / rollout / refund tx assembly) extracted from [[hydrozoa.multisig.ledger.joint.JointLedger]].
  *
