...
/** The pull half of one liaison link (§8.5 of `design/coil-network.md`): send `GetMsgBatch`es,
  ...
  * *extend* them. The per-lane heavy lifting lives in [[Lane]]; the per-shape batch typing lives in
  * the actor's glue closures; this engine owns only the pull-side state machine.
  *
  * @param initialGet
