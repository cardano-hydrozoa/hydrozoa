...
/** The per-shape batch message types for the three liaison links (§8.5 of
  ...
  * lane the server holds. There is no fat one-size-fits-all batch — each shape carries exactly its
  * own lanes, and per-author lane families are `Map`s keyed by the author's [[HeadPeerNumber]].
  *
