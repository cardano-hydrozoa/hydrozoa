...
object CoilPeerWsTransportEvent:
    ...
    /** The connection to the hub ended without error — a clean close, or the read deadline with no
      * inbound frame. Logged because `once` returning normally is otherwise an unlogged state
      * change: the dialer goes straight back to redialing and the link looks continuously up.
      */
