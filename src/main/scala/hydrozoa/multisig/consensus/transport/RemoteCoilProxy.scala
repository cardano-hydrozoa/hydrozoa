...
/** A drop-in replacement, on a hub head peer, for one coil peer's
  ...
  * batch messages (`Population.New` / `OwnHardAck.Get`) over a [[HubWsTransport]] to the bound coil
  * peer. Local-only request variants are dropped by the transport's `send`.
  */
final class RemoteCoilProxy private (
    coil: CoilPeerNumber,
    transport: HubWsTransport,
...
object RemoteCoilProxy {
    def apply(coil: CoilPeerNumber, transport: HubWsTransport): IO[RemoteCoilProxy] =
