...
/** The set of column families the persistence layer opens.
  *
  * Fourteen CFs in total, grouped as:
  *
  ...
  *     the committed L2 state via `restoreTo` (§R2b). Crucially, `softConfirmed` / `hardConfirmed`
  *     are **derived** as `max(SoftConfirmation.key)` / `max(HardConfirmation.key)` — no marker CF.
