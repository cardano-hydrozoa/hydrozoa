...
/** The set of column families the persistence layer opens.
  *
  * Thirteen CFs in total, grouped as:
  *
  ...
  *   - **Aggregator outputs + JL working data** (4) — `BlockResult` (JL per-block output, keyed by
  ...
  *     reads at `softAcked` to seed each peer's RequestLane resume cursor (§5.3). Crucially,
  *     `softConfirmed` / `hardConfirmed` are **derived** as `max(SoftConfirmation.key)` /
...
enum Cf:
    ...
    case BlockResult, SoftConfirmation, HardConfirmation, RequestHighWater
