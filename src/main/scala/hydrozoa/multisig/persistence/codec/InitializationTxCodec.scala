...
import hydrozoa.lib.cardano.scalus.codecs.json.Codecs.{transactionDecoder, transactionEncoder, transactionOutputDecoder, transactionOutputEncoder, utxoDecoder, utxoEncoder}
...
/** Persistence-layer JSON codec for [[InitializationTx]] — the constructor was made public (was
  ...
  * `multisigRegimeProduced`, `resolvedUtxos`, `tx`, `seedUtxo`, `additionalFundingUtxos`,
  * `changeUtxos`.
  *
  * `additionalFundingUtxos: Utxos` round-trips as `List[Utxo]` (same pattern as `ResolvedUtxos`).
  * `changeUtxos: List[Output]` round-trips through the existing `TransactionOutput` CBOR-hex codec.
  */
