...
import hydrozoa.multisig.persistence.codec.RuleBasedCodecs.{
    ruleBasedTreasuryUtxoDecoder,
    ruleBasedTreasuryUtxoEncoder
}
...
/** Persistence-layer JSON codec for [[FallbackTx]] — single case class but spans both
  * multisig and rule-based domains. `treasuryProduced` is a [[RuleBasedTreasuryUtxo]]
  * (rule-based side); it's persisted in the multisig store as part of the multisig regime's
  * fallback effect data, per the design note ("those post-dated effects, not immediate, they
  * are needed to actually fall back into the rule-based regime").
  *
  * `peerVoteUtxosProduced` is `cats.data.NonEmptyList[Utxo]`; round-trips as a JSON array
  * using cats's `NonEmptyList` Circe codec (which Circe provides automatically once `Utxo` has
  * one).
  */
object FallbackTxCodec:
    ...
    private given nonEmptyListUtxoDecoder: Decoder[NonEmptyList[Utxo]] =
        Decoder.decodeList[Utxo].emap(list =>
            NonEmptyList.fromList(list).toRight("expected non-empty list of Utxo for FallbackTx")
        )
