...
/** A typed lane-CF value: the wire payload `P` plus its [[ArrivalStamp]]. The stamp is per-entry
  ...
  */
final case class LaneValue[P](stamp: ArrivalStamp, payload: P)

/** Framing for the value stored at a lane key.
  *
  * Lane values are encoded as `[arrivalStamp : 12][wirePayload …]`. The [[ArrivalStamp]] is the
