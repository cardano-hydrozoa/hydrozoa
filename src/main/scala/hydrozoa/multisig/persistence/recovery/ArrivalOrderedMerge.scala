...
/** Merge the per-family tails into one totally-ordered stream by [[ArrivalStamp]] — the §5.4
  * interleaving the `ReplayActor` (R3) and the one-by-one oracle (R4) both consume.
  *
  * **Why a total gather-and-sort.** The per-family streams are in fact already sorted by stamp: an
  * inbound entry is persisted in receive-cursor order, which is gap-free and next-expected (CR8),
  * so within a family stamps are non-decreasing in index; own entries are created in index order;
  ...
  * per-family scans would also be correct. We gather-and-sort anyway because it is simpler and the
  ...
  * shares it. The `(cf-ordinal, key-bytes)` tiebreak is therefore load-bearing — within a family it
  * reduces to index order; across families it fixes a deterministic interleaving of same-batch
...
object ArrivalOrderedMerge:
    ...
    /** Merge already-per-family-grouped tails (e.g. from [[FamilyScan.scanFamilies]]) into one
      * stamp-ordered stream.
      */
    def merge(perLane: List[List[RawFamilyEntry]]): List[RawFamilyEntry] =
        mergeAll(perLane.flatten)

    /** Order a flat bag of entries by arrival stamp (then the deterministic tiebreak). */
    def mergeAll(entries: List[RawFamilyEntry]): List[RawFamilyEntry] =
    ...
    /** Unsigned lexicographic order on raw key bytes — byte-for-byte, low index first, comparing
      ...
      * `a(i) - b(i)` would sort `0x80` *before* `0x7f` — the opposite of numeric order. The family
      ...
      * numeric index order — so within one family this tiebreak degrades to ordinary index order,
      * and across families (already split by `cf.ordinal` above) it never actually runs on mixed
    ...
    /** `(generation, monotonicNanos, cf-name)` then unsigned key bytes — see
      ...
      */
    private val entryOrdering: Ordering[RawFamilyEntry] =
        Ordering
            .by[RawFamilyEntry, (Int, Long, String)](e =>
                (e.stamp.generation, e.stamp.monotonicNanos, e.key.cf.name)
            )
            .orElse(Ordering.by[RawFamilyEntry, Array[Byte]](_.key.encode)(unsignedBytesOrdering))
