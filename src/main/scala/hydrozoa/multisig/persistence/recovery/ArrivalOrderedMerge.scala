...
/** Merge the per-lane tails into one totally-ordered stream by [[ArrivalStamp]] — the §5.4
  * interleaving the `ReplayActor` (R3) and the one-by-one oracle (R4) both consume.
  *
  * **Why a total gather-and-sort.** The per-lane streams are in fact already sorted by stamp: an
  * inbound entry is persisted in receive-cursor order, which is gap-free and next-expected (CR8),
  * so within a lane stamps are non-decreasing in index; own entries are created in index order; and
  * since block / stack production is sequential (entry `i+1` depends on `i`), own-creation and
  * remote-receipt stamps interleave monotonically on the spines. So a k-way merge across the
  * per-lane scans would also be correct. We gather-and-sort anyway because it is simpler and the
...
object ArrivalOrderedMerge:
    ...
    /** Merge already-per-lane-grouped tails (e.g. from [[LaneScan.scanLanes]]) into one
      * stamp-ordered stream.
      */
    def merge(perLane: List[List[RawLaneEntry]]): List[RawLaneEntry] =
        mergeAll(perLane.flatten)

    /** Order a flat bag of entries by arrival stamp (then the deterministic tiebreak). */
    def mergeAll(entries: List[RawLaneEntry]): List[RawLaneEntry] =
    ...
    /** Unsigned lexicographic order on raw key bytes — byte-for-byte, low index first, comparing
      ...
      * `a(i) - b(i)` would sort `0x80` *before* `0x7f` — the opposite of numeric order. The lane
      ...
      * numeric index order — so within one lane this tiebreak degrades to ordinary index order, and
      * across lanes (already split by `cf.ordinal` above) it never actually runs on mixed widths.
      * Keeping it identical to the store's comparator means the recovery merge can never disagree
      * with on-disk key order. (The §5.4 stamp is the primary key; this only breaks same-batch
      * stamp ties — see [[ArrivalOrderedMerge]].)
      *
