...
/** A typed journal-CF value: the wire payload `P` plus its [[ArrivalStamp]]. The stamp is per-entry
  ...
  * [[StoreCodec.journalValue]]. See the [[JournalValue$]] companion for the byte framing.
  */
final case class JournalValue[P](stamp: ArrivalStamp, payload: P)

/** Framing for the value stored at a journal key.
  *
  * Journal values are encoded as `[arrivalStamp : 12][wirePayload …]`. The [[ArrivalStamp]] is the
  ...
  *
  * Non-journal CFs (`Markers` / `Snapshots` / `Confirmations` / `Meta`) do **not** use this framing
...
object JournalValue:
    ...
    /** Read the arrival stamp from a framed journal value. Throws on a malformed payload (treated
      * as store corruption — fail safe).
      */
    def stamp(framed: Array[Byte]): ArrivalStamp =
        if framed.length < stampWidth then
            throw new IllegalArgumentException(
              s"framed journal value too short for stamp: ${framed.length} bytes"
            )
        else ArrivalStamp.fromBytes(framed)

    /** Read the wire-codec payload from a framed journal value. Throws on a malformed payload. */
    def payload(framed: Array[Byte]): Array[Byte] =
        if framed.length < stampWidth then
            throw new IllegalArgumentException(
              s"framed journal value too short for payload: ${framed.length} bytes"
            )
