...
/** Deposit pre-screening — Hydrozoa's stage of deposit screening (docs/l2-isomorphism.md), run
  ...
  * an L2 tx, and its `l2Payload` arrives out-of-band, so unlike transactions (which have no
  * pre-screening stage) it is payload-checked and time-gated here:
  *
...
object DepositPreScreening {
    ...
    /** Pre-screen a deposit request: parse the deposit tx and check it pins the `l2Payload`, gate
      * on the accept-by deadline, and derive the ledger-screening reference data.
      */
    def preScreen(
        ...
        for {
            ...
            // payload pinning); extract those steps (or rename it) so pre-screening's checks are
