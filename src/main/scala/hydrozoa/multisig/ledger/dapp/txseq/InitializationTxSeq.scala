...
object InitializationTxSeq {
  ...
  def parse(
    ...
    for {
      ...
      hms = HeadMultisigScript(peerKeys)
      ...
      expectedFallbackTx <- FallbackTx
        .build(config, validators, ftxRecipe)
        .left
  ...
  object Builder {
    ...
    def build(args: Args): Either[Error, InitializationTxSeq] = {
      ...
      val hms = HeadMultisigScript(args.peers)
      ...
      for {
        ...
        config: FallbackTx.Config = FallbackTx.Config(
          cardanoInfo = args.env,
          evaluator = args.evaluator,
          headMultisigScript = hms,
          multisigRegimeUtxo =
        ...
        fallbackTx <- FallbackTx
          .build(config, args.validators, fallbackTxRecipe)
          .left
