...
object DepositRefundTxSeq {
    ...
    // TODO: Curry this so we can partially apply?
    final case class Builder(
        ...
        def build: Either[Builder.Error, DepositRefundTxSeq] = for {
            partialRefundTx <- RefundTx.Builder
                .PostDated(config, validators, refundInstructions, refundValue)
                .partialResult
                .left
            ...
            refundTx <- partialRefundTx
                .complete(depositTx.depositProduced, config, validators)
                .left
                .map(Builder.Error.Refund(_))
