...
import hydrozoa.config.HeadConfig.Fields.HasEquityShares
...
object DeinitTx:
    ...
    type Config = Tx.Builder.Config & HasEquityShares

    final case class Builder[Config <: DeinitTx.Config](
        ...
        private val steps = Steps(residualTreasuryToSpend, config)
        ...
        def build: BuildErrorOr[DeinitTx] = for {
            ...
            res <- ctx
                .finalizeContext(
                  ...
                  SharePayoutsDiffHandler.handler(residualTreasuryToSpend, config.equityShares),
                  evaluator,
                  validators
                )
    ...
    private case class Steps(
        ...
        private def spendTreasury =
            Spend(residualTreasuryToSpend.asUtxo, config.headMultisigScript.witness)
              ...
              residualTreasuryToSpend.multisigRegimeTokenName
                ...
                .map(
                  Mint(
                    ...
                    _,
                    -1L,
                    config.headMultisigScript.witness
                  )
                )
        }
        ...
        def mkPayouts: Either[String, List[TransactionBuilderStep]] = {
            ...
            for {
                equity <- (treasuryNewCoin -~ config.equityShares.totalFallbackDeposit).toCoin.left
