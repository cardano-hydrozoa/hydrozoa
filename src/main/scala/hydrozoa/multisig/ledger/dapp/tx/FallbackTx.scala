...
import hydrozoa.config.HeadConfig.Fields.{HasTallyFeeAllowance, HasVotingDuration}
...
object FallbackTx {
    type Config = Tx.Builder.Config & HasTallyFeeAllowance & HasVotingDuration
    def build[Config <: FallbackTx.Config](
        config: Config,
        recipe: Recipe
