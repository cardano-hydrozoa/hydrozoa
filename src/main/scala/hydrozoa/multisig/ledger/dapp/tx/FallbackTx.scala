...
import hydrozoa.multisig.ledger.dapp.tx.Tx.nonSigningNonValidityChecksValidators
...
object FallbackTx {
  case class Config(headMultisigScript: HeadMultisigScript,
  ...
  def build( // The Initial fallback utxo doesn't have the full head config
             config: FallbackTx.Config,
    ...
    val newTreasuryDatum = UnresolvedDatum(
      headMp = headMultisigScript.policyId,
      disputeId = config.tokenNames.voteTokenName.bytes,
      peers = SList.from(headMultisigScript.requiredSigners.map(_.hash)),
      peersN = headMultisigScript.numSigners,
    ...
    def mkVoteToken(amount: Long): MultiAsset = MultiAsset(
      SortedMap(
        (
          ...
          SortedMap((tokenNames.voteTokenName, amount))
        )
      )
    )
    ...
    val peerVoteUtxos: NonEmptyList[TransactionOutput] = {
      val datums = VD(
        NonEmptyList.fromListUnsafe(headMultisigScript.requiredSigners.map(x => PubKeyHash(x.hash)).toList)
      )
    ...
    def collateralUtxos: NonEmptyList[TransactionOutput] = {
      NonEmptyList.fromListUnsafe(
        headMultisigScript.requiredSigners
          .map(es =>
            Babbage(
    ...
    ////////////////////////////////////////////////////
    // Define steps
    val spendHMRW: Spend = Spend(config.multisigRegimeUtxo.asUtxo, headMultisigScript.witness)
    ...
    val mintVoteTokens = Mint(
      headMultisigScript.policyId,
      assetName = tokenNames.voteTokenName,
      amount = headMultisigScript.numSigners + 1L,
      witness = NativeScriptWitness(NativeScriptAttached, Set.empty)
    )
    ...
    val disputeTreasuryAddress = RuleBasedTreasuryScript.address(cardanoInfo.network)
    ...
    val setMetaData = ModifyAuxiliaryData(_ =>
      Some(
        MD.apply(
          Fallback(RuleBasedTreasuryScript.address(cardanoInfo.network))
        )
      )
    )
    ...
    for {
      unbalanced <- TransactionBuilder.build(network = cardanoInfo.network, steps = steps)
      ...
      FallbackTx(
        ...
        validityStart = Slot(setStartSlot.slot).toQuantizedInstant(cardanoInfo.slotConfig),
