...
import scalus.cardano.ledger.{AuxiliaryData, KeepRaw, Metadatum, OriginalCborByteArray, Transaction, TransactionInput, Word64, given}
 ...
 * waste as many resources searching through the transaction in order to determine whether it's malformed or not.
 *
 ...
 * - A cbor-enbcoded payload, represented as a Metadatum.List of Metadatum.Bytes, where each element is at most
 *   64 bytes long.
 *
 ...
 * {
 * $CIP67HeadTag: {
 * $TransactionTypeName: [ $cborChunk1, $cborChunk2, $cborChunk3, ...]
 * }
 * }
 */
...
object Metadata {
    ...
    /**
     * Create the auxiliary data for a specific transaction by cbor-encoding the desired metadata and
     * chunking it into a list of bytes, where each element is no more that 64 bytes long.
     * @param txType
     * @return
     */
    ...
    def parse(txSerialized: Array[Byte])(using protocolVersion: ProtocolVersion): Either[ParseError, L1TxTypes] =
        for {
    ...
    def parse(tx: Transaction)(using protocolVersion: ProtocolVersion): Either[ParseError, L1TxTypes] =
        parse(tx.auxiliaryData)

    def parse(mbAuxData: Option[KeepRaw[AuxiliaryData]])(using protocolVersion: ProtocolVersion): 
    ...
    private def parseMetadataMap(mdMap: Metadatum.Map)(using protocolVersion: ProtocolVersion): Either[ParseError, L1TxTypes] =
