...
class HeadConfigTest extends AnyFunSuite with ScalaCheckPropertyChecks:
    ...
    val vkey1: VerificationKeyBytes = VerificationKeyBytes(
      ByteString.fromHex("01234567890123456789012345678901234567890123456789012345678901234567")
    )
    val vkey2: VerificationKeyBytes = VerificationKeyBytes(
      ByteString.fromHex("11234567890123456789012345678901234567890123456789012345678901234567")
    )
    val vkey3: VerificationKeyBytes = VerificationKeyBytes(
      ByteString.fromHex("21234567890123456789012345678901234567890123456789012345678901234567")
    )
    ...
    test("HeadConfig.parse should fail with too many peers") {
        val manyVKeys = (0 until 255).map { i =>
            VerificationKeyBytes(
              ByteString.fromHex(
                f"${i % 256}%02x234567890123456789012345678901234567890123456789012345678901234567"
              )
            )
        }.toList
