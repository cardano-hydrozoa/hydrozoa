...
class LaneInboundTest extends AnyFunSuite {
    ...
    test("restoreCursor sets the cursor to next(last received), so stale re-serves are rejected") {
        val lane = contiguousFrom(0)
        // Recovered: we durably received up to 5. The cursor resumes at 6 — only NEW items.
        lane.restoreCursor(Some(5)).unsafeRunSync()
    ...
    test("restoreCursor with nothing received leaves the cold initial cursor") {
        val lane = contiguousFrom(0)
        lane.restoreCursor(None).unsafeRunSync()
        assert(lane.cursor.unsafeRunSync() == 0)
    }

    test("restoreCursor on a sparse lane resumes at the remote's next led item") {
        val lane = sparseRemote
        // Last received remote-led item = 3; the remote's next led item is 5.
        lane.restoreCursor(Some(3)).unsafeRunSync()
