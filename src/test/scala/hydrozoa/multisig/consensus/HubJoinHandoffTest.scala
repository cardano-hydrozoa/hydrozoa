...
/** The hub's half of the join exchange: what [[PeerLiaisonHubToCoil]] does when a coil peer's link
  ...
  *
  * ⚠️ **Why this suite exists at all.** `LiaisonProtocol.HubLiaisonMessage` is a union type, and
...
class HubJoinHandoffTest extends AnyFunSuite {
    ...
    private def awaitAnswer(
        seen: Ref[IO, Vector[LiaisonProtocol.CoilLiaisonMessage]]
    ): IO[Unit] =
        def go: IO[Unit] =
            seen.get.flatMap(v =>
    ...
    ): Vector[LiaisonProtocol.CoilLiaisonMessage] = {
    ...
    private def offers(
        out: Vector[LiaisonProtocol.CoilLiaisonMessage]
