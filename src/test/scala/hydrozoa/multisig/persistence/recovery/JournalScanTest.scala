...
/** Unit tests for [[JournalScan]] — that it returns a journal's tail in index order from the
  * cursor, recovers the arrival stamp, and stops at the satellite peer boundary (the §7.1
  * multiplexing).
  */
class JournalScanTest extends AnyFunSuite:

    private def stampN(n: Int): ArrivalStamp = ArrivalStamp(1, n.toLong)

    /** Put a framed journal value (`[stamp : 12][payload …]`) at `key` with an arbitrary payload.
      */
    private def putLane(backend: BackendStore[IO], key: JournalKey, stamp: ArrivalStamp): IO[Unit] =
        val framed = JournalValue.frame(stamp, s"payload-$key".getBytes("UTF-8"))
        backend.put(key.cf, key.encode, framed)

    test("scan returns a spine journal from the cursor (inclusive) to the end, in index order") {
        withStore { backend =>
            val keys = List(0, 1, 2, 3).map(n => JournalKey.Block(BlockNumber(n)))
            for
    ...
    test("scan stops at the satellite peer boundary — does not bleed into the next author") {
        withStore { backend =>
            ...
            val p0Keys = List(0, 1, 2).map(n => JournalKey.HardAck(peer0, HardAckNumber(n)))
            val p1Keys = List(0, 1).map(n => JournalKey.HardAck(peer1, HardAckNumber(n)))
            for
    ...
    test("scan from a mid-journal cursor skips earlier indices") {
        withStore { backend =>
            val peer = PeerId.Head(HeadPeerNumber(2))
            val keys = List(0, 1, 5, 9).map(n => JournalKey.HardAck(peer, HardAckNumber(n)))
            for
                _ <- keys.traverse(k => putLane(backend, k, stampN(0)))
                entries <- JournalScan.scan(backend, JournalKey.HardAck(peer, HardAckNumber(2)))
            yield assert(
              entries.map(_.key) == List(5, 9).map(n => JournalKey.HardAck(peer, HardAckNumber(n)))
            )
        }
    }

    test("scan of an empty journal is empty") {
        withStore { backend =>
            JournalScan.scan(backend, JournalKey.Block(BlockNumber(0))).map(e => assert(e.isEmpty))
        }
    }

    test("payloadBytes round-trips the framed wire payload") {
        withStore { backend =>
            val key = JournalKey.Block(BlockNumber(0))
            for
    ...
    test("round-trip: derive -> scanJournals -> merge over a full multi-peer multi-journal store") {
        withStore { backend =>
            ...
            // scanJournals uses the lower (confirmed) spine floors, so the fast anchor and the acked
            ...
            // For each journal: a few indices below its floor (must be skipped) and a few at/above it
            ...
            val below: List[JournalKey] = List(
              JournalKey.Block(BlockNumber(0)),
              JournalKey.Block(BlockNumber(2)),
              JournalKey.Stack(StackNumber(0)),
              JournalKey.Request(HeadPeerNumber(0), RequestNumber(4)),
              JournalKey.Request(HeadPeerNumber(200), RequestNumber(1)),
              JournalKey.SoftAck(HeadPeerNumber(0), SoftAckNumber(2)),
              JournalKey.SoftAck(HeadPeerNumber(200), SoftAckNumber(2))
            )
            val above: List[JournalKey] = List(
              JournalKey.Block(BlockNumber(3)),
              JournalKey.Block(BlockNumber(4)),
              JournalKey.Stack(StackNumber(2)),
              JournalKey.Stack(StackNumber(3)),
              JournalKey.Request(HeadPeerNumber(0), RequestNumber(5)),
              JournalKey.Request(HeadPeerNumber(1), RequestNumber(0)),
              JournalKey.Request(HeadPeerNumber(200), RequestNumber(2)),
              JournalKey.SoftAck(HeadPeerNumber(0), SoftAckNumber(3)),
              JournalKey.SoftAck(HeadPeerNumber(1), SoftAckNumber(3)),
              JournalKey.SoftAck(HeadPeerNumber(200), SoftAckNumber(4)),
              JournalKey.HardAck(PeerId.Head(HeadPeerNumber(0)), HardAckNumber(0)),
              JournalKey.HardAck(PeerId.Head(HeadPeerNumber(1)), HardAckNumber(1)),
              JournalKey.HardAck(PeerId.Head(HeadPeerNumber(200)), HardAckNumber(0))
            )
            ...
            for
                _ <- seed.traverse { case (k, stamp) =>
                    backend.put(
                      k.cf,
                      k.encode,
                      JournalValue.frame(stamp, s"v-$k".getBytes("UTF-8"))
                    )
                }
                perJournal <- JournalScan.scanJournals(backend, cursors)
                merged = ArrivalOrderedMerge.merge(perJournal)
            yield
