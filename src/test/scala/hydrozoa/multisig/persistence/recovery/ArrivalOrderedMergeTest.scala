...
import hydrozoa.multisig.persistence.{ArrivalStamp, JournalKey, JournalValue}
...
class ArrivalOrderedMergeTest extends AnyFunSuite with ScalaCheckPropertyChecks:
    ...
    private val genKey: Gen[JournalKey] = Gen.oneOf(
      Gen.choose(0, Int.MaxValue).map(n => JournalKey.Block(BlockNumber(n))),
      Gen.choose(0, Int.MaxValue).map(n => JournalKey.Stack(StackNumber(n))),
      for p <- Gen.choose(0, 255); n <- Gen.choose(0L, (1L << 40) - 1)
      yield JournalKey.Request(HeadPeerNumber(p), RequestNumber(n)),
      for p <- Gen.choose(0, 255); n <- Gen.choose(0, Int.MaxValue)
      yield JournalKey.SoftAck(HeadPeerNumber(p), SoftAckNumber(n)),
      for p <- Gen.choose(0, 255); n <- Gen.choose(0, Int.MaxValue)
      yield JournalKey.HardAck(PeerId.Head(HeadPeerNumber(p)), HardAckNumber(n))
    )

    private val genEntry: Gen[RawJournalEntry] =
        for
            key <- genKey
            stamp <- genStamp
    ...
    test("merge of per-journal groups equals merge of the flattened bag") {
        forAll(Gen.listOf(Gen.listOf(genEntry))) { perJournal =>
            assert(
              ArrivalOrderedMerge.merge(perJournal).map(project)
                  == ArrivalOrderedMerge.mergeAll(perJournal.flatten).map(project)
            )
        }
    }
    ...
    test("generation is the high-order key — a later process sorts after an earlier one") {
        ...
        val earlierProcess = RawJournalEntry(
          JournalKey.Block(BlockNumber(0)),
          ArrivalStamp(1, 100L),
          JournalValue.frame(ArrivalStamp(1, 100L), Array.emptyByteArray)
        )
        val laterProcess = RawJournalEntry(
          JournalKey.Block(BlockNumber(1)),
          ArrivalStamp(2, 1L),
          JournalValue.frame(ArrivalStamp(2, 1L), Array.emptyByteArray)
        )
    ...
    private def entryAt(key: JournalKey, stamp: ArrivalStamp): RawJournalEntry =
        RawJournalEntry(key, stamp, JournalValue.frame(stamp, Array.emptyByteArray))
    ...
    test("tiebreak: identical stamp, same CF orders by unsigned key bytes") {
        ...
        val lo = entryAt(JournalKey.Request(peer, RequestNumber(1)), stamp)
        val hi = entryAt(JournalKey.Request(peer, RequestNumber(2)), stamp)
    ...
    test("tiebreak: unsigned byte order — high peer byte sorts after low (not signed)") {
        ...
        val low = entryAt(JournalKey.SoftAck(HeadPeerNumber(1), SoftAckNumber(0)), stamp)
        val high = entryAt(JournalKey.SoftAck(HeadPeerNumber(200), SoftAckNumber(0)), stamp)
