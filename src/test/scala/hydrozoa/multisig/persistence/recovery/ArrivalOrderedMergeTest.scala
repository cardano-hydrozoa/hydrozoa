...
import hydrozoa.multisig.persistence.{ArrivalStamp, LaneKey, LaneValue}
import org.scalacheck.Gen
import org.scalatest.funsuite.AnyFunSuite
import org.scalatestplus.scalacheck.ScalaCheckPropertyChecks

/** Property tests for [[ArrivalOrderedMerge]] — the §5.4 stamp-ordered interleaving of lane tails.
  */
class ArrivalOrderedMergeTest extends AnyFunSuite with ScalaCheckPropertyChecks:
    ...
    private val genKey: Gen[LaneKey] = Gen.oneOf(
      Gen.choose(0, Int.MaxValue).map(n => LaneKey.Block(BlockNumber(n))),
      Gen.choose(0, Int.MaxValue).map(n => LaneKey.Stack(StackNumber(n))),
      for p <- Gen.choose(0, 255); n <- Gen.choose(0L, (1L << 40) - 1)
      yield LaneKey.Request(HeadPeerNumber(p), RequestNumber(n)),
      for p <- Gen.choose(0, 255); n <- Gen.choose(0, Int.MaxValue)
      yield LaneKey.SoftAck(HeadPeerNumber(p), SoftAckNumber(n)),
      for p <- Gen.choose(0, 255); n <- Gen.choose(0, Int.MaxValue)
      yield LaneKey.HardAck(HeadPeerNumber(p), HardAckNumber(n))
    )

    private val genEntry: Gen[RawLaneEntry] =
        for
            key <- genKey
            stamp <- genStamp
    ...
    test("merge of per-lane groups equals merge of the flattened bag") {
    ...
    test("generation is the high-order key — a later process sorts after an earlier one") {
        ...
        val earlierProcess = RawLaneEntry(
          LaneKey.Block(BlockNumber(0)),
          ArrivalStamp(1, 100L),
          LaneValue.frame(ArrivalStamp(1, 100L), Array.emptyByteArray)
        )
        val laterProcess = RawLaneEntry(
          LaneKey.Block(BlockNumber(1)),
          ArrivalStamp(2, 1L),
          LaneValue.frame(ArrivalStamp(2, 1L), Array.emptyByteArray)
        )
    ...
    private def entryAt(key: LaneKey, stamp: ArrivalStamp): RawLaneEntry =
        RawLaneEntry(key, stamp, LaneValue.frame(stamp, Array.emptyByteArray))
    ...
    test("tiebreak: identical stamp, same CF orders by unsigned key bytes") {
        ...
        val lo = entryAt(LaneKey.Request(peer, RequestNumber(1)), stamp)
        val hi = entryAt(LaneKey.Request(peer, RequestNumber(2)), stamp)
    ...
    test("tiebreak: unsigned byte order — high peer byte sorts after low (not signed)") {
        ...
        val low = entryAt(LaneKey.SoftAck(HeadPeerNumber(1), SoftAckNumber(0)), stamp)
        val high = entryAt(LaneKey.SoftAck(HeadPeerNumber(200), SoftAckNumber(0)), stamp)
