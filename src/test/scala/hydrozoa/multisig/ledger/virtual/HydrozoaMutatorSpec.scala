...
import test.{TestPeer, l2EventWithdrawalFromInputsAndPeer}
...
object HydrozoaMutatorSpec extends Properties("Hydrozoa Mutator") {
    ...
    val props: mutable.Seq[Prop] = mutable.Buffer.empty[Prop]
    ...
    val _ = property("Alice can withdraw her own deposited utxos") =
        forAll(genL2EventGenesisFromPeer(Alice)) { genesisEvent =>
            val outState = {
                ...
                for state <- HydrozoaTransactionMutator.transit(
                      Config.fromL1Context(emptyContext),
                      genesisState,
                      l2EventWithdrawalFromInputsAndPeer(
                        TaggedSortedSet.from(genesisState.activeUtxos.keySet),
                        Alice
                      )
                    )
                yield state
