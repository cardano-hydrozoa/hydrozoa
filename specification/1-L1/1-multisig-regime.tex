...
\begin{enumerate}
  ...
  \item The last 28 bytes of the asset name are equal to the Blake2b-224 hash (\codeMath{\mathcal{H}_{28}}) of the utxo nonce spent in the head's initialization transaction (\cref{h:multisig-init}).
\end{enumerate}
...
\begin{description}
  \item[active utxos.] A 32-byte Merkle root hash (\codeMath{\mathcal{RH}_{32}}) of the L2 ledger's active utxo set, as of the latest major L2 block settled on L1 (\cref{h:multisig-settle}).
