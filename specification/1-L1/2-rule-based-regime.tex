...
\begin{itemize}
  ...
  \item The last 28 bytes of the asset name are equal to the Blake2b-224 hash (\codeMath{\mathcal{H}_{28}}) of the multisig treasury utxo spent in the head's transition to the rule-based regime (\cref{h:rule-based-transition}).
\end{itemize}
...
\begin{enumerate}
  ...
  \item Create the default vote utxo, containing a vote for the major L2 block's active utxo set:
    \begin{equation*}
      ...
      \end{array}\right\}
    \end{equation*}
  \item For \codeMath{i} iterated from 1 to \code{nPeers}, create a vote utxo with one dispute token and this datum:%
    \begin{equation*}
