...
\begin{description}
  \item[active utxos.] A 32-byte Merkle root hash (\inlineColored{$\mathcal{RH}_{32}$}) of the offchain ledger's active utxo set, as of the latest major snapshot settled onchain (\cref{h:multisig-settle}).
...
% In the offchain consensus protocol, the head parameters are negotiated via the \code{ReqParams} and \code{AckParams} messages.
% The multi-signed initialization transaction is produced via the \code{ReqInit} and \code{AckInit} messages.
...
It also updates the \code{majorVersion} and \code{activeUtxos} fields of the \code{MultisigTreasuryDatum} to the corresponding fields in the major snapshot, while keeping the \code{params} field unchanged.
...
% The offchain consensus protocol produces the multi-signed settlement and rollout transactions for every major snapshot confirmed by the peers via the \code{ReqMajor} and \code{AckMajor} messages (\cref{h:offchain-consensus-protocol}).
