...
If the peers stop responding to each other in the offchain consensus protocol, the rule-based regime ensures that their funds can still be withdrawn from the Hydrozoa treasury according to the latest confirmed block.
...
\begin{itemize}
  \item The dispute resolution scripts (\cref{h:rule-based-dispute-resolution}) arbitrate the peers' dispute about the latest offchain block confirmed by the offchain consensus protocol before it stalled.
  \item The treasury spending validator (\cref{h:rule-based-treasury}) awaits resolution and then manages withdrawals from the head's treasury based on the resolved offchain block.
\end{itemize}
...
\begin{description}
  ...
  \item[version.] The full version of the resolved offchain block, which includes both the major and minor version numbers. This replaces the \code{majorVersion} field of the multisig treasury.
\end{description}
...
\begin{description}
  ...
  \item[vote status.] Either no vote, a vote to abstain, or a vote for a minor offchain block.
  \item[active utxos.] A 32-byte Merkle root hash (\inlineColored{$\mathcal{RH}_{32}$}) of the offchain ledger's active utxo set, as of the block voted by the peer.
  \item[minor version.] The minor version of the minor offchain block voted by the peer. This block's major version must match the major version that the treasury had when it transitioned to the rule-based regime.
\end{description}
...
\begin{enumerate}
  ...
  \item Let \code{disputeId} be the CIP-67 prefix for 3477, concatenated with the Blake2b-224 hash (\inlineColored{$\mathcal{H}_{28}$}) of the \code{multisigTreasury} output reference.
  ...
  \item Create the rule-based treasury utxo (\code{ruleBasedTreasury}), containing the funds and beacon token from \code{multisigTreasury} and this datum:
    ...
    \end{split}
    \end{equation*}
  \item Create the default vote utxo, containing a vote for the major offchain block's active utxo set:
    \begin{equation*}
      ...
      \end{array}\right\}
    \end{equation*}
  \item For \inlineColored{$i$} iterated from 1 to \code{nPeers}, create a vote utxo with one dispute token and this datum:%
    \begin{equation*}
...
During the voting period of a dispute, each peer gets one opportunity to cast their vote on the latest minor offchain block confirmed by the offchain consensus protocol before it stalled.
          ...
          \item Let \code{voteRedeemer} be an \emph{optional} redeemer argument with this type:
            ...
            \begin{split}
              \T{VoteRedeemer} &\coloneq \left\{
                \begin{array}{lll}
                  \T{minorBlock} &::& \T{MinorBlock} \\
                  \T{multisig} &::& [\T{Signature}]
                \end{array}\right\}\\
              \T{MinorBlock} &\coloneq \left\{
              \begin{array}{lll}
                \T{activeUtxos} &::& \mathcal{RH}_{32} \; \T{UtxoSet} \\
                \T{majorVersion} &::& \T{Int} \\
                \T{minorVersion} &::& \T{Int}
            ...
            \end{split}
            \end{equation*}
          \item If \code{voteRedeemer} is provided, then the \code{multisig} field of \code{voteRedeemer} must have signatures of the \code{minorBlock} field of \code{voteRedeemer} for all the public keys in the \code{peers} field of \code{treasury}.
        \end{enumerate}
...
The treasury spending validator awaits the dispute's resolution and adopts the active utxo set of the resolved offchain block.
...
\begin{description}
  ...
  \item[Deinit.] De-initialize the head, removing all traces of it.
    ...
    \begin{enumerate}
      ...
      \item All \code{headMp} tokens in \code{treasury} with CIP-67 prefixes of \inlineColored{3477} and \inlineColored{4937} must be burned.
    \end{enumerate}
\end{description}
