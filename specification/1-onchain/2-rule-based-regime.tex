...
% TODO clarify that only minor snapshots matching the major snapshot of transition are eligible.
...
\begin{description}
    ...
    \begin{itemize}
      \item Verify the vote utxo spent input:
        \begin{enumerate}
          ...
          \item The \code{vote} field of \code{inputVote} must be empty.
      ...
      \item Verify the treasury reference input:
        \begin{enumerate}[resume]
          ...
          \item \code{disputeId} must match the \code{disputeId} field of \code{treasury}.
      ...
      \item Verify the snapshot redeemer:
        \begin{enumerate}[resume]
          \item Let \code{voteRedeemer} be a redeemer argument with this type:
            \begin{equation*}
            \begin{split}
            ...
            \end{split}
            \end{equation*}
          \item The \code{multisig} field of \code{voteRedeemer} must have signatures of the \code{minorSnapshot} field of \code{voteRedeemer} for all the public keys in the \code{peers} field of \code{treasury}.
        \end{enumerate}
      \item Verify the vote utxo output:
        \begin{enumerate}[resume]
          \item Let \code{outputVote} be an output with the same number of \code{(headMp, disputeId)} tokens as \code{inputVote}. It must not hold any other non-ADA tokens.
          \item The \code{vote} field of \code{outputVote} must match on the \code{activeUtxos} and \code{minorVersion} fields with \code{voteRedeemer}.
        \end{enumerate}
    \end{itemize}
  \item[Tally.] Refer validation to the staking validator of the dispute address.
    ...
    \begin{enumerate}
      ...
      \item Let \code{inputVoteOutref} be the output reference of the input being spent.
      \item The transaction's redeemers list must show \code{tallyValidator} being invoked to withdraw 0 with a redeemer that mentions the tx hash of \code{inputVoteOutref}.
    \end{enumerate}
