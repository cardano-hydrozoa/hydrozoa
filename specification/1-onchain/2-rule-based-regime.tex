...
If the peers stop responding to each other in the L2 consensus protocol, the rule-based regime ensures that their funds can still be withdrawn from the Hydrozoa treasury according to the latest confirmed block.
...
\begin{itemize}
  \item The dispute resolution scripts (\cref{h:rule-based-dispute-resolution}) arbitrate the peers' dispute about the latest L2 block confirmed by the L2 consensus protocol before it stalled.
...
\begin{description}
  ...
  \item[version.] The full version of the resolved L2 block, which includes both the major and minor version numbers. This replaces the \code{majorVersion} field of the multisig treasury.
\end{description}
...
\begin{description}
  ...
  \item[active utxos.] A 32-byte Merkle root hash (\inlineColored{$\mathcal{RH}_{32}$}) of the L2 ledger's active utxo set, as of the block voted by the peer.
...
\begin{enumerate}
  ...
  \item Let \code{votingDeadline} be the voting deadline for this dispute, as defined by the L2 consensus for this transition to the rule-based regime.
