...
A withdrawal is an offchain ledger event that spends one or more utxos in \code{activeUtxos} and creates identical utxos (with the same output references) in \code{withdrawnUtxos}.
...
These new utxos correspond to the deposits absorbed by the settlement transaction (\cref{h:multisig-settle}) of the major offchain block, with the \code{Output} fields of each new utxo set as follows:
\begin{description}
...
\begin{description}
  \item[id.] Set equal to the Blake2b-256 hash (\inlineColored{$\mathcal{H}_{32}$}) of the transaction ID of the major offchain block's settlement transaction.
  \item[index.] Set equal to the onchain deposit's index among the absorbed deposits of the major offchain block, in ascending order of onchain \code{OutputReference}, starting from zero.
\end{description}
