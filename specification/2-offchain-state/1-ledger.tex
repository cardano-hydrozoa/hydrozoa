...
These new utxos correspond to the deposits absorbed by the settlement transaction (\cref{h:multisig-settle}) of the major snapshot, with the \code{Output} fields of each new utxo set as follows:
\begin{description}
...
\begin{description}
  \item[id.] Set equal to the Blake2b-256 hash (\inlineColored{$\mathcal{H}_{32}$}) of the transaction ID of the major snapshot's settlement transaction.
  \item[index.] Set equal to the onchain deposit's index among the absorbed deposits of the major snapshot, in ascending order of onchain \code{OutputReference}, starting from zero.
\end{description}
