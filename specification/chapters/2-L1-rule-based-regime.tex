...
\begin{description}
  ...
  \item[Withdraw.] Withdraw treasury funds and update its active utxo set accordingly.
    Conditions:
    \begin{enumerate}
      \item Let \code{withdrawals} be a list of triples in the content of \code{WithdrawRedeemer}:
          \begin{equation*}
            \T{WithdrawRedeemer} \coloneq \left[\left\{
            \begin{array}{lll}
              \T{key} &::& \T{OutputRef} \\
              \T{utxo} &::& \T{Output^{L2}} \\
              \T{proof} &::& \T{Proof}
          ...
          In each triple:
            \begin{itemize}
              ...
              \item The \code{utxo} is an output with addresses translated to L1 network.
      ...
      \item Starting with the \code{utxosActive} field in \code{treasuryInput}, folding over \code{withdrawals} with the Merkle library's \code{delete} function must produce a result matching the \code{utxosActive} field of \code{treasuryOutput}:%
        \footnote{The details of this function depend on the Merkle library that Hydrozoa implementation will use.
        For example, see the \inlineColored{delete} function in the Merkle Patricia Forestry library \citep{MatthiasBenkortMerklePatriciaForestry2024}.}
        \begin{equation*}
        \begin{split}
        ...
        \end{split}
        \end{equation*}
      \item All \code{withdrawnUtxos} are outputs of the transaction:
         \begin{equation*}
         \begin{split}
            &\T{withdrawnUtxos} :: [Output^{L2}] \\
            &\T{withdrawnUtxos} \coloneq \; \T{map} \; \T{utxo} \; \T{withdrawals}
         \end{split}
         \end{equation*}
