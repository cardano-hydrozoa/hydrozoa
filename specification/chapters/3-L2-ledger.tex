...
The rules for Hydrozoa's L2 transactions are the same as Cardano's ledger rules for L1 transactions,%
...
The hydra node software rejects (with an error message) L2 transactions that violate these requirements, while peers simply ignore \code{ReqTx} messages (\cref{h:l2-consensus-transaction}) that broadcast such transactions.
...
These new utxos correspond to the deposits absorbed by the settlement transaction (\cref{h:l1-multisig-settlement-rollout}) of the major L2 block, with the \codeMathTt{Output^{L2}} fields of each new utxo set as follows:
\begin{description}
