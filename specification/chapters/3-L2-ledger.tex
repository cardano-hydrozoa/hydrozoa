...
\begin{equation*}
\begin{split}
  \T{UtxoSet^{L2}} &\coloneq \T{Map(OutputRef, Output^{L2})} \\
    &\coloneq \left\{
...
The new utxos in \code{activeUtxos} correspond to the deposits absorbed by the settlement transaction, with the \codeMathTt{Output^{L2}} fields of each new utxo set as follows:
\begin{description}
...
\begin{description}
  \item[id.] Set equal to the Blake2b-256 hash (\codeMath{\mathcal{H}_{32}}) of the transaction ID of the major L2 block's settlement transaction.
