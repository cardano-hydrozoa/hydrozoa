...
The effect of this event is to empty the \code{utxosWithdrawn} set and add new utxos to the \code{utxosActive} set, mirroring the effect of the major block's L1 settlement and rollout transactions on the head's utxo state (\cref{h:l1-multisig-settlement-rollout}):
\begin{itemize}
  \item The new L2 utxos added to the \code{utxosActive} set correspond to the L1 deposits absorbed by the L1 settlement transaction.
  \item The L2 utxos emptied from the \code{utxosWithdrawn} set correspond to the L1 utxos paid out by the L1 settlement and rollout transactions.
\end{itemize}

Let \code{depositsAbsorbed} be the set of L1 deposits' output references, in the order that they are spent by the L1 settlement transaction.
Each of the new L2 utxos added by the L2 settlement event must uniquely correspond to an element of \code{depositsAbsorbed} as follows:
\begin{description}
  \item[output reference:]~
    \begin{description}
      \item[id.] Set equal to the Blake2b-256 hash (\codeMath{\mathcal{H}_{32}}) of \code{depositsAbsorbed}.
      \item[index.] Set equal to the L1 deposit's positional index in \code{depositsAbsorbed}.
    \end{description}
  \item[output:]~
    \begin{description}
      \item[addr.] Set equal to the \code{address} field in the corresponding L1 deposit's datum.
      \item[value.] Set equal to the value of the corresponding L1 deposit.
      \item[datum.] Set equal to the \code{datum} field in the corresponding L1 deposit's datum.
      \item[script.] Empty.
    \end{description}
  \end{description}
