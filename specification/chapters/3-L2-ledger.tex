...
\begin{description}
  ...
  \item[withdrawnUtxos.] The set of utxos that have been withdrawn from \code{activeUtxos} and can be settled on L1 in the multisig regime.
...
An L2 transaction spends one or more utxos in \code{activeUtxos}, replacing them with zero or more new utxos.
...
The effect of this event is to empty the \code{withdrawnUtxos} set and add new utxos to the \code{activeUtxos} set, mirroring the effect of the major block's L1 settlement transaction on the head's utxo state (\cref{h:l1-multisig-settlement-rollout}).
Correspondingly, the L2 settlement event's identifier is the Blake2b-256 hash (\codeMath{\mathcal{H}_{32}}) of the transaction ID of the major block's L1 settlement transaction.

To model the effect of an L2 settlement on the L2 \code{activeUtxos} set, we represent it as a L2 transaction with all fields omitted except the outputs (\codeMathTt{Tx^{L2S} \subseteq Tx^{L2}}).
These outputs must correspond to the deposits absorbed by the L1 settlement transaction, as follows:
\begin{description}
...
The \code{OutputReference} fields of each of these outputs is as follows:
\begin{description}
  \item[id.] Set equal to the L2 settlement event's identifier.
  \item[index.] Set equal to the L1 deposit's index among the absorbed deposits of the major L2 block, in ascending order of L1 \code{OutputReference}, starting from zero.
\end{description}
