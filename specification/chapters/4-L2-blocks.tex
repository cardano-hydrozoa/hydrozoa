...
The \code{creationTime} field of a block indicates the time when it was created.
...
\begin{enumerate}
  \item Initialize variables and arguments (immutable by default):
    \begin{enumerate}
      ...
      \item Let \code{settlementEvent} be a mutable variable initialized to an empty \codeMathTt{Settlement^{L2}}.
    \end{enumerate}
  \item For each L2 event \code{x} in \code{eventsSeenL2} in ascending order of \code{timeReceived}:
      \begin{enumerate}
        \item If \code{x.eventType} is \code{Transaction}, apply \code{x.event} to \code{localStateL2.utxosActive} using Hydrozoa's ledger rules for L2 transactions (\codeMathTt{Tx^{L2}}).
          If it is valid:
          \begin{enumerate}
          ...
          \end{enumerate}
        \item If \code{x.eventType} is \code{Withdrawal}, apply \code{x.event} to \code{localStateL2.utxosActive} using Hydrozoa's ledger rules for L2 withdrawals (\codeMathTt{Tx^{L2W}}).
          If it is valid:
          \begin{enumerate}
            \item Append (\code{x.eventId}, \code{x.eventType}) to \code{block.eventsValid}.
            \item Update \code{localStateL2.utxosActive} to the result of this transition.
            \item Insert the spent inputs of \code{x.event} into \code{localStateL2.utxosWithdrawn}.
            \item Insert the spent inputs of \code{x.event} into \code{settlementEvent.utxosPaidOut}.
          \end{enumerate}
        \item Otherwise, insert (\code{x.eventId}, \code{x.eventType}) into \code{block.eventsInvalid}.
      \end{enumerate}
  \item Move all utxos from \code{localStateL2.utxosWithdrawn} to \code{settlementEvent.utxosPaidOut}.
  \item If \code{finalizing} is \code{False}, for each deposit \code{d} in \code{stateL1.depositUtxos}:
    \begin{enumerate}
      \item Insert \code{d} into \code{block.depositsAbsorbed} if:
        \begin{equation*}
          \T{timeCurrent} \in [\T{d.deadline} + \T{depositMarginMaturity}, \T{d.deadline} - \T{depositMarginExpiry})
        \end{equation*}
      \item Insert this utxo into \code{settlementEvent.utxosAdded}:
        \begin{equation*}
          \left\{
          \begin{array}{lll}
  ...
  \item If \code{finalizing} is \code{True}:
    \begin{itemize}
      \item Move all utxos from \code{localStateL2.utxosActive} to \code{settlementEvent.utxosPaidOut}.
    \end{itemize}
  \item Set \code{block.blockType} according to the first among these conditions to hold:
    \begin{enumerate}
      ...
      \item \code{Major} if \code{settlementEvent.utxosAdded} is non-empty.
      \item \code{Major} if \code{settlementEvent.utxosPaidOut} is non-empty.
      \item \code{Major} if:
        \begin{equation*}
...
\begin{enumerate}
  \item Initialize variables and arguments (immutable by default):
    \begin{enumerate}
      ...
      \item Let \code{localStateL2} be a mutable variable initialized to \code{stateL2}.
    \end{enumerate}
  \item Return \framebox{\code{Invalid}} if:
    \begin{equation*}
    ...
    \begin{enumerate}
      ...
      \item If \code{x.eventType} is \code{Transaction}, apply \code{x.event} to \code{localStateL2.utxosActive} using Hydrozoa's ledger rules for L2 transactions (\codeMathTt{Tx^{L2}}):
        \begin{enumerate}
          ...
          \item Otherwise, update \code{localState.utxosActive} to the result of this transition.
        \end{enumerate}
      \item If \code{x.eventType} is \code{Withdrawal}, apply \code{x.event} to \code{localStateL2.utxosActive} using Hydrozoa's ledger rules for L2 withdrawals (\codeMathTt{Tx^{L2W}}):
        \begin{enumerate}
          \item If it is invalid, return \framebox{\code{Invalid}}.
          \item Otherwise, do all of these:
            \begin{itemize}
              \item Update \code{localState.utxosActive} to the result of this transition.
              \item Insert the spent inputs of \code{x.event} into \code{localStateL2.utxosWithdrawn}.
              \item Insert the spent inputs of \code{x.event} into \code{settlementEvent.utxosPaidOut}.
            \end{itemize}
        \end{enumerate}
  ...
  \item For each event \code{y} in \code{eventsInvalid}:
    \begin{enumerate}
      ...
      \item If \code{y.eventType} is \code{Transaction}, apply \code{y.event} to \code{localStateL2.utxosActive} using Hydrozoa's ledger rules for L2 transactions (\codeMathTt{Tx^{L2}}). If it is valid, return \framebox{\code{Invalid}}.
      \item If \code{y.eventType} is \code{Withdrawal}, apply \code{y.event} to \code{localStateL2.utxosActive} using Hydrozoa's ledger rules for L2 withdrawals (\codeMathTt{Tx^{L2W}}). If it is valid, return \framebox{\code{Invalid}}.
    \end{enumerate}
  % TODO ! Remove this sillyness:
  \item Move all utxos from \code{localStateL2.utxosWithdrawn} to \code{settlementEvent.utxosPaidOut}.
  \item If \code{finalizing} is \code{False}, for each deposit \code{d} in \code{depositsAbsorbed}:
    \begin{enumerate}
      ...
      \item Otherwise, let \code{d'} be the corresponding deposit in \code{stateL1.depositUtxos}.
      \item Return \framebox{\code{Invalid}} if:
        \begin{equation*}
  ...
  \item Return \framebox{\code{Invalid}} if any of these fails to hold:
    \begin{enumerate}
      ...
      \item \code{block.utxosActive} matches the Merkle root hash of \code{localStateL2.utxosActive}.
