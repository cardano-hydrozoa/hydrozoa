...
Let \code{utxosActive}, \code{utxosAdded}, and \code{utxosWithdrawn} be the sets produced by the block creation or validation procedure for a given block (\code{block}) and block validation environment (\code{env}).
Upon confirmation of \code{block} by the peers, it immediately affects \code{env} as follows:
\begin{enumerate}
  \item Update \code{env.stateL2.utxosActive} to \code{utxosActive}.
  \item For each L2 event in \code{block.eventsValid}, set the \code{blockNum} of the corresponding event in \code{env.eventsSeenL2} to \code{block.blockNum}.
  \item If \code{utxosAdded} is non-empty, then append this genesis event to \code{env.eventsSeenL2}:
    \begin{equation*}
...
\begin{description}
  \item[settlement.] The L1 settlement transaction (\cref{h:l1-multisig-settlement-rollout}) that absorbs the block's \code{absorbedDeposits} and pays out as many utxos corresponding to the block's \code{utxosWithdrawn} as fit within Cardano's L1 transaction constraints.
  \item[rollouts.] The L1 rollout transactions (\cref{h:l1-multisig-settlement-rollout}) that pay out utxos corresponding to the rest of the block's \code{utxosWithdrawn}.
