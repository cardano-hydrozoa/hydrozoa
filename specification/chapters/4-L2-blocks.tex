...
\begin{description}
  ...
  \item[params.] The head's parameters:
    \begin{description}
      ...
      \item[multisig regime timeout.] If this non-negative time duration has passed since the latest confirmed major block's creation, the head can fall back to the rule-based regime with this block's major version.
    \end{description}
...
\begin{description}
  ...
  \item[post-dated fallback.] An L1 transaction that transitions the multisig treasury to the rule-based regime with the block's major version (\cref{h:l1-rule-based-fallback}).
