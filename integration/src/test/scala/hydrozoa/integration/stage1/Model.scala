...
val logger = Logging.logger("Stage1.Model")
...
private val blockCycleLens: Lens[ModelState, BlockCycle.InProgress] =
    Lens[ModelState, BlockCycle.InProgress](
...
given ModelCommand[L2TxCommand, Unit, ModelState] with {
    ...
    override def runState(cmd: L2TxCommand, state: ModelState): (Unit, ModelState) =
        ...
        logger.trace(s"OUTPUT finalState.blockCycle event IDs: ${finalEvents.map(_._1.eventId)}")
            ...
            case BlockCycle.InProgress(_, creationTime, prevVersion, events) =>
                ...
                val newState = state.copy(
                  ...
                  depositEnqueued = state.depositEnqueued.filterNot(cmd =>
                      (result.depositsAbsorbed ++ result.depositsRefunded).contains(cmd._1.eventId)
                  )
                )
                result -> newState
    ...
    private def mkBlockBrief(
        ...
        val depositsAbsorbed = depositEnqueued
            .filter(cmd => {
                ...
                val depositAbsorptionStart = txTiming.depositAbsorptionStartTime(submissionDeadline)
...
given ModelCommand[RegisterDepositCommand, Unit, ModelState] with {
    override def runState(
        ...
        logger.debug(s"MODEL>> RegisterDepositCommand for event ID: ${cmd.registerDeposit.eventId}")
