...
object RBRHlNetTest extends Properties("RBRHlNet"):
    ...
    // Example assertions as one Prop — deterministic, no generators, so each is checked once.
    ...
    val _ = property(
      "BallotStatus order matches maxVote precedence: Abstained < Awaiting < Voted"
    ) = {
        val order = summon[Order[BallotStatus]]
        allTrue(
          order.lt(Abstained, Awaiting),
          order.lt(Awaiting, Voted),
          order.lt(Abstained, Voted)
        )
    }

    val _ = property("the net assembles and is well-sorted") = allTrue(
    ...
    val _ = property("Ballot domain: the listing excludes self-links and non-Voted versions") = {
        ...
        allTrue(
          ...
          !valid.contains(box(1, 1, Voted, 1)), // non-terminal self-link
          !valid.contains(box(1, 2, Awaiting, 1)) // non-Voted box with a version
        )
    }

    val _ = property("Owner domain: the listing is exactly the ownership diagonal") = {
        val valid = RBRHlNet.validOwners(nHeadPeers = 3)
        allTrue(
          valid == Set(peer0 -> BigInt(1), peer1 -> BigInt(2), peer2 -> BigInt(3)),
          !valid.contains(peer1 -> BigInt(1)) // peer1 does not own box key 1
        )
    }
    ...
    val _ = property(
      "Vote: the pending box flips to Voted(v); owner, collateral, references restored"
    ) = {
        val n = fired(net, RBRTransitionId.Vote, voteMode(net, peer0, 1, 2, 1))
        allTrue(
          ballotCount(n, box(1, 2, Awaiting, 0)) == SafeLong(0),
          ballotCount(n, box(1, 2, Voted, 1)) == SafeLong(1),
          n.placesMap(RBRPlaceId.Owner).marking.get((peer0, BigInt(1))) == SafeLong(1),
          n.placesMap(RBRPlaceId.Collateral).marking.get(peer0) == SafeLong(1),
          n.placesMap(RBRPlaceId.UnresolvedTreasury).marking.get(()) == SafeLong(1)
        )
    }

    val _ = property("Vote: only the box's owner can vote it") =
        // peer1 does not own box key=1 (peer0 does) — the Owner read fails
        allTrue(!net.isModeEnabled(RBRTransitionId.Vote, voteMode(net, peer1, 1, 2, 1)))

    val _ = property("Vote: a peer cannot vote twice") = {
        val n = fired(net, RBRTransitionId.Vote, voteMode(net, peer0, 1, 2, 1))
        allTrue(!n.isModeEnabled(RBRTransitionId.Vote, voteMode(n, peer0, 1, 2, 2)))
    }

    val _ = property("VotingDeadline: closes voting for Vote and RatchetVote, opens tallying") = {
        ...
        allTrue(
          !n.isModeEnabled(RBRTransitionId.Vote, voteMode(n, peer1, 2, 3, 1)),
          !n.isModeEnabled(RBRTransitionId.RatchetVote, ratchetMode(n, peer2, 1, 2, Voted, 1, 2)),
    ...
    val _ = property("Abstain: has no validity window — enabled even after the deadline") = {
        ...
        allTrue(
          ballotCount(n2, box(2, 3, Abstained, 0)) == SafeLong(1),
          ballotCount(n2, box(2, 3, Awaiting, 0)) == SafeLong(0)
        )
    }

    val _ = property(
      "RatchetVote: Voted boxes ratchet strictly monotonically, by any acting peer"
    ) = {
        val n0 = fired(net, RBRTransitionId.Vote, voteMode(net, peer0, 1, 2, 1))
        // vOld=1 → vNew=2, acted by peer2 (not the owner)
        val n = fired(n0, RBRTransitionId.RatchetVote, ratchetMode(n0, peer2, 1, 2, Voted, 1, 2))
        allTrue(
          ballotCount(n, box(1, 2, Voted, 2)) == SafeLong(1),
          ballotCount(n, box(1, 2, Voted, 1)) == SafeLong(0),
    ...
    val _ = property(
      "RatchetVote: an Abstained box ratchets from version 0; an Awaiting box cannot ratchet"
    ) = {
        ...
        val n =
            fired(n0, RBRTransitionId.RatchetVote, ratchetMode(n0, peer1, 1, 2, Abstained, 0, 1))
        allTrue(
          ballotCount(n, box(1, 2, Voted, 1)) == SafeLong(1),
    ...
    val _ = property("Tally: adjacency is required; tallying is disabled before the deadline") = {
        val n = fired(net, RBRTransitionId.VotingDeadline, Binding.empty)
        allTrue(
    ...
    val _ = property(
      "Tally: maxVote winner selection — status precedence, version order, tie to removed"
    ) = {
        ...
        allTrue(
          ballotCount(a, box(0, 2, Voted, 0)) == SafeLong(1),
          ballotCount(b1, box(0, 2, Voted, 2)) == SafeLong(1),
    ...
    val _ = property("Tally: full fold converges to the single box (0,(0,(Voted,vmax)))") = {
        val n = fullyTallied
        val ballots = n.placesMap(RBRPlaceId.Ballots).marking.multiplicityMap
        allTrue(
          ballots.size == 1,
          ballotCount(n, box(0, 0, Voted, 2)) == SafeLong(1)
        )
    }

    val _ = property("Resolution: not enabled until the fully-tallied box exists") =
        // the terminal (0, 0, Voted, ·) box does not exist before the fold completes
        allTrue(!net.isModeEnabled(RBRTransitionId.Resolution, resolutionMode(net, 0, peer0)))

    val _ = property("Resolution: consumes the tallied box and flips the treasury to Resolved") = {
        val n =
            fired(fullyTallied, RBRTransitionId.Resolution, resolutionMode(fullyTallied, 2, peer0))
        allTrue(
          // the tallied box is spent — ballots empty
          n.placesMap(RBRPlaceId.Ballots).marking.multiplicityMap.isEmpty,
          // treasury flips Unresolved → Resolved
          n.placesMap(RBRPlaceId.UnresolvedTreasury).marking.get(()) == SafeLong(0),
          n.placesMap(RBRPlaceId.ResolvedTreasury).marking.get(()) == SafeLong(1),
          // collateral spent and recreated; script/regime references read back
          n.placesMap(RBRPlaceId.Collateral).marking.get(peer0) == SafeLong(1),
          n.placesMap(RBRPlaceId.TreasuryScriptRef).marking.get(()) == SafeLong(1)
        )
    }

    val _ = property(
      "Resolution: reveals the resolved version via the ResolvedVersion selector"
    ) = {
        val n =
            fired(fullyTallied, RBRTransitionId.Resolution, resolutionMode(fullyTallied, 2, peer0))
        allTrue(n.placesMap(RBRPlaceId.ResolvedVersion).marking.get(BigInt(2)) == SafeLong(1))
    }

    val _ = property("Evacuation: not enabled until Resolution reveals the version") =
        // no ResolvedVersion token yet — the selector cannot bind `version`, so no batch is proposed
        allTrue(!enabledVia(fullyTallied, RBRTransitionId.Evacuation))

    val _ = property(
      "Evacuation: drains the resolved version's whole batch, leaving other SECs' committed"
    ) = {
        ...
        val batch = version2.toSeq.flatMap { output =>
            Seq(
              obligations.get((BigInt(2), output)) == SafeLong(0),
              evacuated.get((BigInt(2), output)) == SafeLong(1)
            )
        }
        allTrue(
          (batch ++ Seq(
            ...
            obligations.get((BigInt(1), committedOutputs(1).head)) == SafeLong(1),
            // treasury stays Resolved; the selector persists; collateral recreated
            n.placesMap(RBRPlaceId.ResolvedTreasury).marking.get(()) == SafeLong(1),
            n.placesMap(RBRPlaceId.ResolvedVersion).marking.get(BigInt(2)) == SafeLong(1),
            n.placesMap(RBRPlaceId.Collateral).marking.get(peer0) == SafeLong(1)
          ))*
        )
    }

    val _ = property("Evacuation: drains the whole resolved batch in one firing, then is spent") = {
        ...
        allTrue(!enabledVia(once, RBRTransitionId.Evacuation))
    }

    val _ = property("Deinit: not enabled while a resolved-version obligation remains") =
        // resolved but not yet evacuated — the emptiness inhibitor blocks Deinit
        allTrue(!resolved.isModeEnabled(RBRTransitionId.Deinit, deinitMode(resolved, 2, peer0)))

    val _ = property(
      "Deinit: after evacuation, spends the treasury and regime, tearing down the head"
    ) = {
        val evacuated = firedVia(resolved, RBRTransitionId.Evacuation)
        // the obligations are drained, so Deinit is now enabled
        val deinitEnabled =
            evacuated.isModeEnabled(RBRTransitionId.Deinit, deinitMode(evacuated, 2, peer0))
        ...
        allTrue(
          ...
          n.placesMap(RBRPlaceId.ResolvedTreasury).marking.get(()) == SafeLong(0),
          n.placesMap(RBRPlaceId.RegimeRef).marking.get(()) == SafeLong(0),
          n.placesMap(RBRPlaceId.Collateral).marking.get(peer0) == SafeLong(1)
        )
    }
