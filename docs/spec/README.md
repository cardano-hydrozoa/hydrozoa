...
| [head-params-hash.md](head-params-hash.md) | `headParamsHash`: one blake2b digest over the whole head config, pinned in the multisig treasury datum. The preimage layout, `l2ParamsHash` as an opaque nested leaf, what is deliberately excluded, and the five checks that enforce it. |
