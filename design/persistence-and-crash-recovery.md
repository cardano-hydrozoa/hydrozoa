...
- **Recover:** `nextSeq = max(HubHardAck.key where hub == own) + 1` — derived from
  ...
  `RequestSequencer`'s `max(own Request) + 1`). The per-coil-peer marks are read from the
