...
| [init-tx-parsing.md](init-tx-parsing.md) | Implementing | GUM-129: the head **parses** its initialization tx from the config (a bare Cardano tx + in-tx metadata) instead of **re-building** it; the init-tx builder moves out to a bootstrapping submodule. Adds `totalEquity` to the init-tx metadata (self-contained tx), derives the fallback via `FallbackTx.Build`, drops the fallback + build-time bookkeeping from the config, evaluates the tx on read (utxo existence deferred to `CardanoLiaison`). |
